...
Options:
  ...
  -A, --all-excludes                    Songs must match all exclude filter criteria to be excluded.
	-D METHOD, --duplicates METHOD        Delete duplicate songs.
																				METHOD can be OLD/NEW for deleting all but the newest/oldest song respectively.
...
def main():
	...
	if cli['duplicates'] and cli['duplicates'] not in ['OLD', 'NEW']:
		print(__doc__)
		sys.exit()
	...
	if cli['duplicates']:
		def duplicate_gen(songs):
			cmp_fields = ['albumArtist', 'album', 'title', 'artist', 'trackNumber']

			def cmp_values(song):
				return tuple(map(song.get, cmp_fields))
			songs = sorted(
				filter(
					lambda song: all(key in song for key in (cmp_fields + ['lastModifiedTimestamp'])),
					songs
				),
				key=lambda song: cmp_values(song) + (datetime.fromtimestamp(int(song['lastModifiedTimestamp']) / 10**6),),
				reverse=cli['duplicates'] == 'NEW'
			)
			for i, j in zip(songs, songs[1:]):
				if cmp_values(i) == cmp_values(j):
					yield j
		songs_to_delete = list(duplicate_gen(songs_to_delete))
